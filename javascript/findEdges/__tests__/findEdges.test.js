...
const findEdge = require('../findEdges');
...
describe('Get Edge Test', () => {
  it('should return true and the weight if a path exists', () => {
    expect(findEdge(graph, [Pandora, Metroville])).toEqual('true, $82');
    expect(findEdge(graph, [Arendelle, Monstropolis, Naboo])).toEqual('true, $115');
  });
  it('should retrun false if a path doesn\'t exist', () => {
    expect(findEdge(graph, [Naboo, Pandora])).toBe('false, $0');
    expect(findEdge(graph, [Narnia, Arendelle, Naboo])).toBe('false, $0');
  });
  it('should return false for invalid nodes', () => {
    expect(findEdge(graph, [Naboo, Herbalife])).toBe('false, $0');
  });
});
